...
# %%
def load_and_analyze_jsonl(file_path, output_csv):
    matching_events = []
    all_argument_texts = []
...
def analyze_and_print_results(matching_events):
    # Analyze event types
    event_type_counter = Counter([event['event_type'] for event in matching_events])

    print(f"Event types and their frequencies:")
    for event_type, count in event_type_counter.most_common():
        print(f"{event_type}: {count}")

    print(f"\nTotal number of events: {len(matching_events)}")
    print(f"Number of unique event types: {len(event_type_counter)}")

    # Analyze roles
    role_counter = Counter([event['role'] for event in matching_events])

    print(f"\nRoles and their frequencies:")
    for role, count in role_counter.most_common():
        print(f"{role}: {count}")

    print(f"\nNumber of unique roles: {len(role_counter)}")
    ...
    print(f"\nTop 20 event type and role combinations:")
...
# %%
# Set the file path
file_path = "/home/aleksi/Projects/war_tg_qa/TextEE/data/processed_data/wikievents/all.json"
output_csv = "wikievents_events.csv"

# %%
# Example usage
#search_terms = ["Russia"]  # You can change this to any string or list of strings
matching_events, all_argument_texts = load_and_analyze_jsonl(file_path, output_csv)
analyze_and_print_results(matching_events)
